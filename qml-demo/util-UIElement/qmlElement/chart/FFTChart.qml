...
//        console.log(JSON.stringify(data))
        if( channelId_ === chart.channelId && !chart.isChangeChannel ){
            if( chart.times_ == 0 ){
                lineChart.frameLen = chart.unitLength = data["frameLength"];
//                lineChart.dt = chart.dt = data["dt"];
                lineChart.xAxisMax = chart.xmax = data["frameLength"] * data["deltaFreq"]
                lineChart.df = chart.unit = data["deltaFreq"];
                lineChart.rePaint();
                chart.dataArr = new Array(chart.unitLength).fill(0);
                chart.times = 0;
            }

            chart.addData( data["values"] , 1 , data["frameLength"] );
    ...
    function addData( data , count , frameLen ){
        ...
        for( var k = 0; k < count; k++ ){
            ...
            {
                 chart.dataArr[i] = data[i];
                // chart.dataArr[i] = (chart.dataArr[i] * chart.times + data[k][i] * 10 ) / ( chart.times + 10 );
//                chart.dataArr[i] = (chart.dataArr[i] * n + data[k][i] ) / ( n + 1 );  // cal average
